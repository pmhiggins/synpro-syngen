...
**Structure**
The program is divided in _x_ blocks:
...
D. Complement and trascribe: A biopython block that will obtain the complementary chain and transcribe the input DNA.
...
H. DNA concentration: This block will generate some miscelaneous values of interest such as average lenght of the gene in case there are more than one sequence, the total weight of the genetic material and the DNA concentration in mol/L for the reaction quotient
...
**Note**
This program needs a environment called reaktoro to run properly.
You will need to install miniconda or conda to use different environments and then install reaktoro.
Once having conda installed type:
conda install reaktoro
Or go to the reaktoro page for further instructions


Using Biopython:
If you have Biopython you can run the code normally. To install biopython before running the program type: pip install Biopython
...
5. It needs biopython to run at the moment. The normal python reader is not very accurate in the AA count.


**Location**
The program can be saved in any folder/directory.

**Test files**
There is a file called testcell.fasta with 6 protein sequences, 4 real and 2 control sequences, that can be used to test the program.

**Complications**
There is a problem in the module F to read the dictionary in the reaction quotient.
